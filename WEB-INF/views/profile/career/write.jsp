...
//			document.querySelector("#imDate").visibility='hidden';				// 비활성화 >> 안됨
// 			document.getElementById("#end_date").value = "null";				// end_date 입력 안했을 떄 null 값 입력
// 			document.getElementById("#end_date").value = '';					// end_date 입력 안했을 떄 null 값 입력
// 			document.getElementById('#end_date').value = "2000/00/00";
		}
...
<form action="${pageContext.request.contextPath }/profile/career_write" method="post" name="write">
	<div align="center">
		<div>회사명
			<input type="text" name="company" placeholder="ex) 네이버">
		</div><br>
		<div>직책
			<input type="text" name="posi" placeholder="ex) 팀장">json 데이터 활용 예정
		</div><br>
		<div>직무
			<input type="text" name="task" placeholder="ex) 웹디자이너">json 데이터 활용 예정
		</div><br>
		<div>고용형태
